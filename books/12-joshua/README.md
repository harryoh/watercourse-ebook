...
- **특이사항**: 여호수아 33장. 차례 `[N]` 대괄호 + 페이지 러닝헤더/꼬리말 제거.

## 재생성 / 빌드
```bash
# 1) 원고 재생성(원본에서 다시 뽑을 때만)
python3 shared/lib/extract_hwp.py books/12-joshua   # (기존 manuscript.md 보존; --force로 덮어쓰기)
# 2) 표지(필요 시)
python3 shared/lib/make_cover.py books/12-joshua
# 3) 검토
python3 shared/lib/review.py books/12-joshua
# 4) EPUB 빌드
./build.sh books/12-joshua
```
