...
- **특이사항**: 전도서 23장. 표준 추출기. 제목 "물 위에 던진 떡".

## 재생성 / 빌드
```bash
# 1) 원고 재생성(원본에서 다시 뽑을 때만)
python3 shared/lib/extract_hwp.py books/05-ecclesiastes   # (기존 manuscript.md 보존; --force로 덮어쓰기)
# 2) 표지(필요 시)
python3 shared/lib/make_cover.py books/05-ecclesiastes
# 3) 검토
python3 shared/lib/review.py books/05-ecclesiastes
# 4) EPUB 빌드
./build.sh books/05-ecclesiastes
```
