...
- **제목**: 아모스 강해  /  **저자**: 조춘숙 목사
- **성경/주제**: 아모스
- **장 수**: 본문 9개 (전체 섹션 12개: 머리말·판권·교회안내 포함)
...
- **특이사항**: 아모스 9장. 점선형 차례(신형식 기준 책).

## 재생성 / 빌드
```bash
# 1) 원고 재생성(원본에서 다시 뽑을 때만)
python3 shared/lib/extract_hwp.py books/17-amos   # (기존 manuscript.md 보존; --force로 덮어쓰기)
# 2) 표지(필요 시)
python3 shared/lib/make_cover.py books/17-amos
# 3) 검토
python3 shared/lib/review.py books/17-amos
# 4) EPUB 빌드
